...
### If a Stage B dispatch fails startup validation, admission, or otherwise runs no work

A startup-validation failure creates zero jobs, and an `admission` refusal creates no deploying jobs, so nothing was deployed and no attestation exists. Treat the approved SHA window as spent anyway, and recover in this exact order.
