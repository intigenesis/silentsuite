...
export function buildPublicServedAttestation({ admissionBytes, expectedPublicSha, expectedSourcePrivateSha, expectedSourceArtifact, expectedPublicRepository, privateAdmissionHmacKey, publicServedHmacKey, publicDeploymentRun, publicArtifact, publicArtifactName, deploymentVerified, clientServedAt, now = () => new Date().toISOString().replace(/\.\d{3}Z$/, 'Z') }) {
  ...
  const admission = verifyPrePublicAdmission({ admissionBytes, expectedAdmissionDigest: admissionDigest, expectedPublicSha, expectedSourcePrivateSha, expectedSourceArtifact, expectedPublicRepository, hmacKey: privateAdmissionHmacKey })
